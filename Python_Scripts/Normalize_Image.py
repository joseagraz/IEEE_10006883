...
def Initialize():
    ...
    OutputFilePath            = str(Path(InputArguments.Output_Directory) / IMAGE_OUTPUT / NewFileName)
...
def UseHistogramMatching(ImageToTransformDensity, PixelColorArray,PixelCountArray):
    ...
    MatchedHistogramDensity = match_histograms(ImageToTransformDensity,\
                                               UnraveledData,\
                                               multichannel=False)
    return MatchedHistogramDensity
#------------------------------------------------------------------------------------------------
...
def NormalizeImage(InputImage,NormalizingHistogram,NormalizingStainVectors):    
    ...
    if ExtractedPixels.size:
        ...
        ExtractedPixels      = np.where(ExtractedPixels == 0, 1/WHITE_COLOR, ExtractedPixels)
        print('Computing -1 * Log')
        ODImage              = -1 * np.log(ExtractedPixels / WHITE_COLOR)
        
        print('-----------------------------------')
        print('Find Stain Vectors using Image Optical Density')        
        SlideStainVectors    = FindStainVectors(ODImage.compute().T)
        ...
        DensityMapDask         = StainDensityMapDask[:, StainLocation]
        print('{}: Set Negative Values to 0    '.format(StainName))
        StainMaskDask          = DensityMapDask > 0
        print('{}: Unravel Mask Indeces        '.format(StainName))
        StainMaskIndexesDask   = np.argwhere(StainMaskDask).flatten()
        print('{}: Apply Mask                  '.format(StainName))
        TrimmedDensityMapDask  = DensityMapDask[StainMaskDask]
        ...
        TransformedStainDask   = UseHistogramMatching(TrimmedDensityMapDask,Bins,Data)
        print('{}: Apply new stain densities  '.format(StainName))
        DensityMapDask[StainMaskIndexesDask] = TransformedStainDask
        print('{}: Check for negative numbers '.format(StainName))
        TransformedHematoxylin = np.where(DensityMapDask < 0,0,DensityMapDask)
        ...
        DensityMapDask[StainMaskIndexesDask] = TransformedStainDask
        print('{}: Check for negative numbers '.format(StainName))
        TransformedEosin      = np.where(DensityMapDask < 0,0,DensityMapDask)
